...
* 子类和父类的属性方法重名时，为了访问父类成员，需要用super进行访问如果没有重名，super，this，直接访问的效果都是一样的
...
* 特点：编译类型看左边，运行类型看右边，可以调用父类的所有成员(需要遵守访问权限)，不能调用子类的特有成员，可以调用父类里有的在子类里重写的方法。最终运行效果看子类的具体实现，运行方法按照从子类开始查找方法，然后调用
...
* 要求父类的引用必须指向的是当前目标类型的对象，要求父类的类型原先就要是指向子类型
```
//要求
...
person[i].say();//运行类型会根据实际情况由jvm判断
