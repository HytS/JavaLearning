...
无论程序怎么运行，Singleton类只有一个实例化对象存在；
### this、super
this作用：1、表示类中的属性；2、使用this调用本类的构造方法；3、表示当前对象；this只能出现在构造器的第一行；可以使用this()调用无参构造器；this调用构造方法一定要留一个出口，即程序中至少存在一个构造方法是不使用this调用其他构造方法的，一般将无参构造方法当作出口，即无参构造器最好不要去调用其他构造方法；
...
## final
final修饰的变量为常量，常量不可以修改，final修饰的方法不可以被重写；final修饰的类不可以被继承，final修饰的属性一定要赋值，赋值后不可以更改；final修饰对象，值可变，引用不可变；常量在编译阶段会把使用常量的地方换成真实的字面量；全局变量：public static final;
...
方法重写时从private变成default不算方法重写（父类的一个方法定义为private，子类的方法定义为default）；重写时子类的访问权限要大于等于父类的访问权限；
...
main方法由虚拟机调用；jvm调用main时不用创建对象；main方法中可以直接调用main所在类的静态变量和方法，但不能直接访问非静态成员；如果一个方法想要被主方法调用，必须按以下格式声明：public static 返回值 方法名(){}
java常用内存区域：栈空间内存：保存所有的对象名称；堆内存空间：保存每个对象的具体属性值；全局数据区：保存static类型的属性；全局代码区：保存所有的方法定义；
static方法定义的说明：此方法不希望通过实例化对象调用，此时会有两种因素（本类没有提供属性，实例化对象没有意义；本类无法直接进行对象实例化，只能利用static操作）
### 引用传递
...
### 抽象类
在java中也可以创建一种类专门用来当父类，称为抽象类；其目的就是要设计者根据它的格式来修改并创建新的类，但是不能直接由抽象类创建对象，只能通过抽象类派生出新的类，再由它创建对象；
抽象类不能被final修饰，因为抽象类要被子类重写；
一个类没有足够的信息去描述对象；抽象类不能被实例化，要被继承；使用场景：当子类需要完成一定功能，而每个子类功能又不同，可以将方法定义为抽象方法；构造方法和静态方法不能定义为抽象；抽象类可以没有抽象方法，类一旦包含抽象方法就要定义为抽象类；抽象类可以定义构造方法；而且抽象类也存在各个属性，所以子类在实例化之前要先对父类进行实例化；
...
内部类唯一好处就是可以方便的访问外部类的私有属性（通过将内部类拿到外面去），同时外部类也可以访问内部类的私有属性；使用static修饰内部类可以使内部类变成外部类，使用static修饰的内部类不能访问非static的外部类属性；

在外部访问内部类：外部类.内部类 内部类对象=外部类实例.new 内部类();首先要找到外部类实例对象，才能通过外部类实例化对象去实例化内部类对象（Outer out=new Outer(); Outer.Inner in= out.new Inner()）；外部类.内部类 内部类对象=外部类实例.new 内部类();
内部类定义后，生成的class文件是以'Outer$Inner.class'

在方法中定义的内部类不能访问方法的参数，如果方法中的参数想要被访问，就要在参数前加final（1.8之后可以不加final）；

内部类可以直接访问私有属性；局部内部类定义在类中的局部位置，通常在方法和代码块中，可以访问外部类的所有成员，不可以添加访问修饰符；

局部内部类访问外部类成员：直接访问；外部类访问局部内部类成员：先创建对象再访问；外部类其他类不能访问局部内部类；其他类访问外部类成员 外部类.this.成员变量
...
每个String对象都是存放在堆内存中；匿名对象就是开辟了堆内存并且可以直接使用的对象；String str1="hello" 在堆中开辟好的堆内存的使用权给了str1；
一个字符串就是一个String类的匿名对象，使用new创建String类对象（String str1=new String("hello")），无论如何都会再开辟一个新空间，所以相当于开辟了两个空间，但是真正使用的只是用new开辟的空间，另一个是垃圾空间
字符串内容不可以更改，实际上是通过内存地址的'断开-连接'变化完成的，本身字符串内容没有变化；字符串的内容不可以改变，能改变的是其内存地址的指向；String作为类的属性存在时，可以改变类的属性内容；
#### String类所采用的模式是共享设计模式
在jvm底层会有一个对象池（不一定只保存String对象），当代码中使用了直接赋值的方式定义了一个String对象时，会将此字符串对象所使用的匿名对象入池保存，而后如果其他String对象也采用了直接赋值的方式且内容相同则不再开辟新的堆内存空间，而是使用已有的对象进行引用的调配；
#### 直接赋值和构造方法赋值的区别
...
### 类的声明周期
类的装载就是通过类加载器把.class二进制文件装入jvm的方法区，并在堆区创建描述该类的java.lang.Class对象，用来封装数据；同一个类只会被jvm加载一次。链接就是把二进制数据组装成可以运行的状态，链接分为校验、准备、解析；校验用来确认此二进制文件是否适合当前的jvm版本；准备就是为静态成员分配内存空间，并设置默认值；解析指的是转换常量池的代码引用为直接引用的过程，直到所有符号引用都可以被运行程序使用。完成之后，类型可以初始化，初始化之后的对象都可以使用，直到一个对象不再使用后，将被垃圾回收，释放空间，当没有任何引用指向Class对象时，将结束类的生命周期
### 工厂设计模式
#### 将反射应用到工厂模式上
#### 结合属性文件的工厂模式
### 反射机制
...
## Annotation
Annotation可以用来修饰类、属性、方法，而且Annotation不影响运行；
### 系统内建的Annotation
@Override 重写的Annotation；@Deprecated 不赞成使用的Annotation；@SuppressWarnings：压制安全警告的Annotation；
@Deprecated主要功能是声明一个不建议使用的方法,如果在程序中使用此方法，编译时将出现警告信息；
@SuppressWarnings主要功能是压制警告；
### 自定义Annotation
#### Annotation的定义格式
public @interface Annotation 名称{数据类型 变量名称() default 默认值;}
要定义Annotation必须以使用@interface的方式，在定义Annotation也可以定义各种变量，但是变量的定义之后要加()
使用@interface就相当于继承了Annotation接口；可以向自定义的Annotation传递参数，在Annotation中必须使用变量接收参数；只要在定义Annotation时设置了属性，就必须在使用的时候设置其内容，也可以在定义Annotation时直到默认值；
在Annotation中，可以使用Retention定义一个Annotation的保存范围，
### 通过反射取得Annotation
### Lambda表达式
Lambda表达式指的是应用在含有一个抽象方法的接口环境下的一种简化定义形式；Lambda对应接口只能有一个方法；匿名内部类对应的接口可以有多个方法；
Lambda表达式语法：(参数)->{方法体}；为了分辨出Lambda表达式的使用接口，可以在接口上使用@FunctionInterface注解声明，这样就表示此为函数式接口，里面只允许定义一个抽象方法和多个default、static方法；
### 方法引用
...
## java内存模型
内存管理实际上是对运行时数据区这段空间的管理；运行时数据区分为方法区、栈、堆、程序计数器、本地方法栈；
### jvm内存体系结构
#### 程序计数器
每个线程都有一个自己独立的程序计数器，只会记录该线程执行顺序，它属于线程私有区域，该区域所占内存空间极小
#### java栈
每个线程都会存在私有的栈内存空间，在线程对象每一次调用一个方法时都会自动产生一个栈帧，并将这些栈帧保存在该栈中；每个栈帧主要组成:本地变量表（方法的局部变量或形参）、操作数栈、指向当前方法所属的类的运行时常量池的引用（引用其他类的常量或使用String池中的字符串）、方法返回地址；
所有方法在执行时都会自动封装成一个栈帧，而后当前执行的方法保存在栈顶。并且每一个方法执行时都会有一个返回地址的信息保存，
#### 本地方法栈
功能于java栈相似，本地方法栈为本地方法服务；
#### 方法区
方法区是所有线程对象共享的区域，在方法区中保存了每一个类的信息，静态变量、常量、常量池信息；
#### 堆
主要保存具体的数据信息，在jvm启动时被创建，此内存空间为所有线程对象共享区域；在java开发中是垃圾收集器的主要管理区域；
#### 直接内存
虚拟机之外的主机内存；
### java对象的访问方式
Object obj=new Object();Object obj存放在jvm的本地变量表中，表示一个引用类型的数据；new Object()一个实例化对象，该数据保存在堆内存中；堆内存中会记录Object类的相关类信息的地址，这些地址所执行的数据会存储在方法区中；
#### 通过引用类型去访问具体对象的方式
通过句柄访问：jvm堆会专门有一块区域用来作为句柄池，存储相关句柄所执行的实例数据地址（包括堆中地址和方法区中的地址）；
#### 通过直接指针访问
reference中存储的就是对象在堆中的实际地址，在堆中存储的对象信息包含了方法区的相应类型数据；
### jvm垃圾收集
在java中所有的垃圾收集问题几乎都是针对堆内存空间完成的；在整个java内存模型中，主要有3个内存区：年轻代、老年代、元空间，同时还有几块动态调整的内存伸缩区；jvm的内存回收就是对这几块空间的回收处理操作；java在每次创建对象时如果发现内存不足时都会自动向其他区域延伸；
### G1收集器
### 引用类型
强引用：当内存不足时，jvm宁可出现超出内存的错误使程序停止，也不会回收此对象来释放空间
软引用：当内存不足时，会回收这些对象的内存，用来实现内存敏感的高速缓存
弱引用：无论内存是否紧张，被垃圾回收器发现立即回收
幽灵引用：和没有任何引用一样

强引用是jvm默认的支持模式；软引用在一般开发中作为数据缓存使用；幽灵引用是为了防止出现哪些无法被回收的对象，幽灵引用最大的特定就是在其内部保存的引用对象，无论何时取得，结果永远都是null；
# javaweb
